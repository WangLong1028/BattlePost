package com.shuo.chatmodule.login;
    ...
    private void loginFailed(String failedMsg) {
        if (loginResultCallback != null) {
            if (failedMsg.equals(NetworkConstant.CONNECT_ERROR))
                loginResultCallback.logFailed("与服务器通讯失败");
            if (failedMsg.equals(NetworkConstant.LOGIN_PASSWORD_ERROR))
                loginResultCallback.logFailed("密码错误");
            if (failedMsg.equals(NetworkConstant.LOGIN_USER_NOT_EXIST_ERROR))
                loginResultCallback.logFailed("用户不存在");
        }
    }
